...
Flags: --file - or <path> (input, default stdin), --inline <json> (small only), --json [fields] (machine-readable stdout, optional field filter), --wait <sec> (poll total timeout, default 600), --round <n> (poll a specific round), --reason <enum> + --detail <text> (cancel).
...
- Exit 76 -> session expired (60-min TTL hit). Unrecoverable; start a new session if we are not done.
- Exit 64 -> your Grilling JSON was malformed or failed schema validation. The CLI prints a hint to stderr -- fix the JSON and retry.
...
2. Wait for my explicit confirmation (ok / confirm / yes / LGTM / proceed). Do not treat silence or vague replies as consent.
3. On confirmation, run grilling-sleek complete <id>. This closes the browser session cleanly.

If I abandon the grilling or you lose context, run grilling-sleek cancel <id> so the session does not linger until TTL.
...
1. Command line arguments: --server http://...
2. Environment variables: GRILLING_SLEEK_SERVER
3. Config file: ~/.config/grilling-sleek/config.json
4. Default value: https://grillingsleek.online
