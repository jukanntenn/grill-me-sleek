...
// src/api.ts
var SERVER = process.env.GS_SERVER ?? "https://grillingsleek.online";
var HTTP_TIMEOUT = Number(process.env.GS_HTTP_TIMEOUT ?? 10) * 1e3;
var LONGPOLL_HTTP_TIMEOUT = Number(process.env.GS_LONGPOLL_HTTP_TIMEOUT ?? 65) * 1e3;
function apiClient(method, isLongPoll = false, idempotencyKey) {
  return distribution_default.create({
    ...
    hooks: {
      beforeRequest: idempotencyKey ? [(req) => req.headers.set("Idempotency-Key", idempotencyKey)] : []
    }
  });
}
...
program2.name("grill").description("grilling-sleek CLI \u2014 push structured questions to a web page").version("0.1.0");
...
program2.command("poll <session_id>").description("Wait for user response (long-poll loop)").option("-r, --round <n>", "Specific round to poll (default: current)").option("-w, --wait <sec>", "Total wait timeout (default: 600)", "600").action(async (sessionId, opts) => {
...
async function pollLoop(sessionId, round, totalWait) {
