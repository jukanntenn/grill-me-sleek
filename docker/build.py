...
def build_frontend(project_root):
    """Build the frontend (web/) to produce web/dist/."""
    web_dir = os.path.join(project_root, "web")
    if not os.path.isdir(web_dir):
        env_error(
            f"Frontend directory not found: {web_dir}",
            "Ensure the web/ directory exists with a valid package.json.",
        )

    logger.info("Building frontend (web/)...")
    result = subprocess.run(
        ["npm", "run", "build"],
        cwd=web_dir,
        capture_output=True,
    ...
    if not os.path.isdir(dist_dir):
        env_error(
            f"Frontend build did not produce {dist_dir}",
            "Check the vite build configuration in web/package.json.",
        )

    logger.info("Frontend build completed: %s", dist_dir)
...
def main():
    ...
    # Build frontend first (produces web/dist/)
