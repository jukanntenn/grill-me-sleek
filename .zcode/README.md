...
delegate all formatting/linting to prek — the single source of truth (the
`format` and `lint` groups in the workspace prek configs). No formatter mapping
lives in these scripts, so they cannot drift from prek/CI.

- `hooks/post_tool_use.py` — PostToolUse (`Edit|Write`): runs
  `prek run --group format --files <edited>`. prek routes the file to the right
  project formatter (cargo fmt / prettier / builtin fixers) at the correct cwd.
  Never blocks; real errors are surfaced to stderr.
- `hooks/stop.py` — Stop: runs `prek run --group lint --all-files` (clippy +
  eslint + tsc); on non-zero exit it prints
