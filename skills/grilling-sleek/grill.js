...
var require_wait = __commonJS({
  "node_modules/.pnpm/thread-stream@4.2.0/node_modules/thread-stream/lib/wait.js"(exports2, module2) {
    "use strict";
    var WAIT_MS = 1e4;
    function wait(state, index, expected, timeout2, done) {
      const max = timeout2 === Infinity ? Infinity : Date.now() + timeout2;
      const check = () => {
    ...
    function waitDiff(state, index, expected, timeout2, done) {
      const max = timeout2 === Infinity ? Infinity : Date.now() + timeout2;
      const check = () => {
        const current = Atomics.load(state, index);
        if (current !== expected) {
...
var require_transport = __commonJS({
  "node_modules/.pnpm/pino@10.3.1/node_modules/pino/lib/transport.js"(exports2, module2) {
    ...
    var { join: join2, isAbsolute, sep: sep2 } = require("node:path");
...
var require_prettify_level = __commonJS({
  "node_modules/.pnpm/pino-pretty@13.1.3/node_modules/pino-pretty/lib/utils/prettify-level.js"(exports2, module2) {
    ...
    function prettifyLevel({ log, context }) {
      ...
      const output2 = getPropertyValue(log, levelKey);
      if (output2 === void 0) return void 0;
      const labelColorized = colorizer(output2, { customLevels, customLevelNames });
      if (prettifier) {
        const [label] = getLevelLabelData(output2);
        return prettifier(output2, levelKey, log, { label, labelColorized, colors: colorizer.colors });
      }
...
var require_prettify_time = __commonJS({
  "node_modules/.pnpm/pino-pretty@13.1.3/node_modules/pino-pretty/lib/utils/prettify-time.js"(exports2, module2) {
    ...
    function prettifyTime({ log, context }) {
      ...
      const output2 = translateFormat ? formatTime(time, translateFormat) : time;
      return prettifier ? prettifier(output2) : `[${output2}]`;
    }
  }
});
...
var require_pino_pretty = __commonJS({
  "node_modules/.pnpm/pino-pretty@13.1.3/node_modules/pino-pretty/index.js"(exports2, module2) {
    "use strict";
...
// node_modules/.pnpm/jsonrepair@3.15.0/node_modules/jsonrepair/lib/esm/utils/JSONRepairError.js
...
// node_modules/.pnpm/jsonrepair@3.15.0/node_modules/jsonrepair/lib/esm/utils/stringUtils.js
var codeSpace = 32;
var codeNewline = 10;
var codeTab = 9;
var codeReturn = 13;
...
// node_modules/.pnpm/jsonrepair@3.15.0/node_modules/jsonrepair/lib/esm/regular/jsonrepair.js
...
function jsonrepair(text) {
  ...
  if (isStartOfValue(text[i]) && endsWithCommaOrNewline(output2)) {
    if (!processedComma) {
      output2 = insertBeforeLastWhitespace(output2, ",");
    }
    parseNewlineDelimitedJSON();
  } else if (processedComma) {
    output2 = stripLastOccurrence(output2, ",");
  }
  while (text[i] === "}" || text[i] === "]") {
    i++;
    parseWhitespaceAndSkipComments();
...
${output2}
      ...
      while (true) {
        if (i >= text.length) {
          ...
          if (!stopAtDelimiter && isDelimiter(text.charAt(iPrev))) {
            i = iBefore;
            output2 = output2.substring(0, oBefore);
  ...
  function parseConcatenatedString() {
    ...
    while (text[i] === "+") {
      ...
      i++;
      parseWhitespaceAndSkipComments();
      output2 = stripLastOccurrence(output2, '"', true);
      const start = output2.length;
      const parsedStr = parseString();
      if (parsedStr) {
        output2 = removeAtIndex(output2, start, 1);
      } else {
        output2 = insertBeforeLastWhitespace(output2, '"');
      }
    }
  ...
  function parseNumber() {
    ...
    if (i > start) {
      ...
      output2 += invalid ? `"${text.substring(start, i)}"` : num;
      return true;
    }
    return false;
  }
  ...
  function parseUnquotedString(isKey) {
    ...
    if (i > start) {
      ...
      output2 += symbol === "undefined" ? "null" : JSON.stringify(symbol);
      if (text[i] === '"') {
        i++;
      }
      return true;
  ...
  function parseRegex() {
    if (text[i] === "/") {
      ...
      i++;
      output2 += JSON.stringify(text.substring(start, i));
      return true;
    }
  }
...
async function exists(filename) {
  return new Promise((resolve) => (0, import_fs.access)(filename, import_fs.constants.F_OK, (error) => resolve(!error)));
}
...
var RotatingFileStream = class extends import_stream.Writable {
  ...
  async reopen(size) {
    let file;
    try {
      file = await (0, import_promises.open)(this.filename, "a", this.options.mode);
    } catch (e) {
      if (e.code !== "ENOENT")
        throw e;
  ...
  async touch(filename) {
    ...
    try {
      ...
      file = await (0, import_promises.open)(filename, "a");
    }
    await file.close();
  ...
  interval() {
    ...
    const set = async () => {
      ...
      if (time <= 0) {
        ...
        this.timeout = (0, import_timers.setTimeout)(set, time > this.maxTimeout ? this.maxTimeout : time);
        this.timeout.unref();
      }
    };
    set();
...
var checks = {
  encoding: (type, options, value) => new import_util.TextDecoder(value),
  immutable: () => {
  },
  initialRotation: () => {
  },
...
function getLogDir() {
  ...
  const xdgStateHome = process.env.XDG_STATE_HOME || (0, import_node_path.join)((0, import_node_os.homedir)(), ".local", "state");
  return (0, import_node_path.join)(xdgStateHome, "grilling-sleek", "logs");
}
...
var fileStream = createStream("grilling-sleek.log", {
  path: logDir,
  size: "10M",
  maxFiles: 30,
  compress: "gzip",
...
function logPoll(event, sessionId, round, extra) {
  logger.debug({
    event: `poll_${event}`,
...
function logRetry(retryCount, backoffMs2, errorMsg) {
  logger.debug({
    event: "retry_backoff",
    retry_count: retryCount,
    backoff_ms: backoffMs2,
    error_msg: errorMsg
  });
}
...
// src/api.ts
var SERVER = process.env.GRILLING_SLEEK_SERVER ?? "https://grillingsleek.online";
var HTTP_TIMEOUT = Number(process.env.GRILLING_SLEEK_HTTP_TIMEOUT ?? 10) * 1e3;
var LONGPOLL_HTTP_TIMEOUT = Number(process.env.GRILLING_SLEEK_LONGPOLL_HTTP_TIMEOUT ?? 65) * 1e3;
function apiClient(method, isLongPoll = false, idempotencyKey) {
  let startTime = 0;
...
async function readInput(fileFlag) {
...
// ../server/schemas/grilling.json
var grilling_default = {
  $schema: "https://json-schema.org/draft/2020-12/schema",
  $id: "Grilling",
  type: "object",
  properties: {
    name: { type: "string", description: "\u4F1A\u8BDD/\u95EE\u5377\u540D\u79F0" },
    description: { type: "string", description: "\u4F1A\u8BDD/\u95EE\u5377\u63CF\u8FF0" },
    additional_notes: {
      type: "object",
      description: "\u5168\u5C40\u8865\u5145\u8BF4\u660E\u6846\u914D\u7F6E",
      properties: {
        label: { type: "string", description: "\u6846\u6807\u7B7E\u6587\u6848\uFF0C\u7F3A\u7701\u8D70 i18n \u9ED8\u8BA4" },
        placeholder: { type: "string", description: "\u5360\u4F4D\u63D0\u793A\uFF0C\u7F3A\u7701\u8D70 i18n \u9ED8\u8BA4" },
        max_length: { type: "integer", description: "\u6700\u5927\u5B57\u7B26\u6570\uFF0C\u4E0D\u8BBE\u5219\u4E0D\u9650" },
        required: { type: "boolean", default: false, description: "true=\u5FC5\u586B" }
      }
    },
    questions: {
      type: "array",
      ...
      items: {
        ...
        properties: {
          id: {
            type: "string",
            description: "snake_case \u7A33\u5B9A\u6807\u8BC6\uFF0C\u56DE\u7B54\u6620\u5C04 key"
          },
          header: { type: "string", description: "UI \u6807\u7B7E" },
          text: { type: "string", description: "\u5B8C\u6574\u95EE\u9898\u63CF\u8FF0" },
          ...
          options: {
            ...
            description: "single/multi \u5FC5\u9700"
          },
          recommended: {
            type: "integer",
            description: "\u63A8\u8350\u9009\u9879\u7684 index"
          },
          variant: {
            type: "string",
            enum: ["default", "yesno", "rating"],
            default: "default"
          },
          rating_max: {
            type: "integer",
            default: 5,
            minimum: 2,
            description: "variant=rating \u65F6\u7684\u4E0A\u9650"
          },
          placeholder: {
            type: "string",
            description: "text \u7C7B\u578B\u7684\u5360\u4F4D\u6587\u672C"
          },
          max_length: {
            type: "integer",
            description: "text \u7C7B\u578B\u7684\u6700\u5927\u957F\u5EA6"
          },
          required: {
            type: "boolean",
            default: true,
            description: "false=\u53EF\u4E0D\u9009/\u53EF\u4E0D\u586B"
          },
          allow_custom_text: {
            type: "boolean",
            default: true,
          ...
          explanation: {
            type: "string",
            description: "\u63A8\u8350\u7406\u7531\uFF0C\u663E\u793A\u5728 recommended \u65C1"
          }
        },
        required: ["id", "header", "text", "type"],
        allOf: [
          {
            if: {
              properties: {
                type: { const: "single" },
                variant: { not: { enum: ["yesno", "rating"] } }
              },
              required: ["type"]
            },
...
var validateGrilling = ajv.compile(grilling_default);
function errExit(message, code) {
...
function parseGrilling(raw) {
  ...
  if (!validateGrilling(parsed)) {
    const errors = validateGrilling.errors?.map((e) => `  ${e.instancePath || "/"}: ${e.message}`).join("\n");
    errExit(`schema validation failed:
${errors}`, 64);
  }
  const ids = /* @__PURE__ */ new Set();
  const grilling = parsed;
...
function output(result, jsonFields, humanReadable) {
  if (jsonFields === null) {
    if (humanReadable) {
...
async function extractHttpError(e) {
...
program2.name("grill").description("grilling-sleek CLI \u2014 push structured questions to a web page").version("0.2.0-rc.1").option("-v, --verbose", "Enable debug logging");
program2.command("create").description("Create a new session with first round of questions").option("-f, --file <path>", "Input file path (default: stdin)").option("-i, --inline <json>", "Inline JSON string").option("-j, --json [fields]", "JSON output (optionally filter fields)").option("-w, --wait <sec>", "Wait for response after creating (seconds)").action(async (opts) => {
  let raw;
  if (opts.inline) {
    raw = opts.inline;
  } else {
  ...
  try {
    const resp = await client.post("sessions", { json: grilling }).json();
...
URL: ${resp.url}`
    ...
    logError("create session failed", e, { session_id: void 0 });
...
program2.command("poll <session_id>").description("Wait for user response (long-poll loop)").option("-r, --round <n>", "Specific round to poll (default: current)").option("-w, --wait <sec>", "Total wait timeout (default: 600)", "600").option("-j, --json [fields]", "JSON output").action(async (sessionId, opts) => {
  let round = opts.round ? Number(opts.round) : void 0;
  if (round === void 0) {
    try {
      const client = apiClient("get");
      const session = await client.get(`sessions/${sessionId}`).json();
...
program2.command("push <session_id>").description("Push the next round of questions").option("-f, --file <path>", "Input file path (default: stdin)").option("-i, --inline <json>", "Inline JSON string").option("-j, --json [fields]", "JSON output").option("-w, --wait <sec>", "Wait for response after pushing").action(async (sessionId, opts) => {
  let raw;
  if (opts.inline) {
    raw = opts.inline;
  } else {
  ...
  const client = apiClient("post", false, idempotencyKey);
  try {
    const resp = await client.post(`sessions/${sessionId}/rounds`, { json: grilling }).json();
    const jsonFields = parseJsonFields(opts.json);
    output(
      resp,
      jsonFields,
...
program2.command("complete <session_id>").description("Complete a session").action(async (sessionId) => {
  try {
    const client2 = apiClient("get");
    const rounds = await client2.get(`sessions/${sessionId}/rounds`).json();
  ...
  try {
    const resp = await client.patch(`sessions/${sessionId}`, { json: { status: "completed" } }).json();
    output(
      resp,
      parseJsonFields(void 0),
...
program2.command("cancel <session_id>").description("Cancel a session").option(
  "--reason <enum>",
  "Cancel reason: user_cancelled | agent_aborted | error",
  "agent_aborted"
).option("--detail <text>", "Additional detail").action(async (sessionId, opts) => {
  ...
  const client = apiClient("patch");
  try {
    const resp = await client.patch(`sessions/${sessionId}`, { json: body }).json();
    output(
      resp,
      parseJsonFields(void 0),
...
program2.command("status <session_id>").description("Get session status").option("-j, --json [fields]", "JSON output").action(async (sessionId, opts) => {
  const client = apiClient("get");
  try {
    const resp = await client.get(`sessions/${sessionId}`).json();
    output(
      resp,
...
function backoffMs(consecutiveErrors) {
  return Math.min(Math.pow(2, consecutiveErrors - 1), 30) * 1e3;
}
async function pollLoop(sessionId, round, totalWait, jsonFields = null) {
  const deadline = Date.now() + totalWait * 1e3;
  ...
  while (Date.now() < deadline) {
    ...
    try {
      ...
      if (status === 429) {
        ...
        logRetry(consecutiveErrors + 1, delaySec * 1e3, `HTTP 429`);
        await sleepMs(Number.isNaN(delaySec) || delaySec <= 0 ? 5e3 : delaySec * 1e3);
        continue;
      }
