...
This DESIGN.md is adapted from Vercel's marketing site for **grilling-sleek**, a single-page questionnaire tool. The visual language (ink-on-canvas, hairline borders, stacked shadows, Inter font, negative tracking) is preserved; marketing-only components are discarded and tool-appropriate components are composed from the primitives.

### Components Used (Mapped to Vercel Tokens)

| Application Component | Vercel Primitive | Styling |
|---|---|---|
| QuestionCard | `card-soft` | `canvas-soft` bg, `rounded.md` 8px, hairline border, `spacing.lg` padding |
| SingleControl (default — radio) | `form-input` | option rows with hairline border; selected = radio indicator ink fill (circle turns ink with on-primary dot); row keeps canvas bg |
| SingleControl (yesno — pills) | `tab-ghost` | `rounded.pill-sm` 64px; selected = ink fill + on-primary text |
| SingleControl (rating — numbered pills) | `tab-ghost` | `rounded.pill-sm` 64px, min-width 48px; selected = ink fill + on-primary text |
| MultiControl (checkbox) | `form-input` | same option-row style as SingleControl(default); selected = checkbox indicator ink fill (box turns ink with on-primary check); row keeps canvas bg |
| TextControl (textarea) | `form-input` | 80px min-height, hairline border, `rounded.sm` 6px |
| AdditionalNotes field | `card-soft` + `form-input` | same chrome as QuestionCard |
| Banner (error + retry) | `ex-toast` | `error-soft` bg + `error-deep` border + retry button |
| TerminalPage | `ex-empty-state-card` | `canvas-soft` bg, `rounded.lg`, `spacing.3xl` padding, centered |
| SubmitButton | `button-primary` | ink bg, on-primary text, `rounded.pill` 100px |
| Controls (theme/language) | `nav-cta-*` | `rounded.sm` 6px, hairline border |
| Recommended mark | `badge-secondary` | `caption` text, muted color, inline |

### Components Discarded (No Application Scenario)

- `hero-band`, `feature-mesh-band`, `showcase-band-light/dark` — marketing bands
- `card-marketing`, `card-marketing-large`, `template-card` — marketing cards
- `pricing-card`, `pricing-card-featured` — pricing tiers
- `code-editor-mockup`, `logo-strip`, `footer` — marketing chrome
- The multi-stop mesh gradient — hero-scale decoration only

### Dark Theme (Polarity Inversion)

Vercel documents light/dark as a polarity-flip concept. The dark theme inverts the ink↔canvas token ladder while keeping semantic colors constant:
...
| `ink` | `#171717` | `#ededed` |
| `error`/`warning`/`success` | unchanged | unchanged |

Applied via `[data-theme="dark"]` CSS variable overrides. Theme selection (light/dark/system) managed by `useTheme` hook with `localStorage` persistence + `matchMedia` tracking.

### Font

Uses **Inter Variable** via `@fontsource-variable/inter` (the documented Geist substitute). Self-hosted as woff2 subsets (CSP `font-src 'self'`).
